/*!
 ...
 - Enhancement: Delay setting Singleton AdUnit inheritance to provide time for queueing a registry prior to sending request to DFP
 - Bug Fix: Singleton divId generation when duplicates exist
 - Bug Fix: QueueSingleton exception when requesting missing divId
 - Bug Fix: For Interval Refreshes, validate interval prior to performing refresh
 - Bug Fix: Check Array.isArray on slots when refreshing instead of checking for typeof === "object"
*/
...
var AdFuel = (function adFuelBuilder() {
    ...
    function queueSingleton(queueOptions) {
        ...
        //this may change regObj.rktr_slot_id or return null if not found
        regObj = _renderCheck(regObj);
        
        if (!regObj) {
            _warnLog('missing singleton: ' + queueOptions.divId);
        } else {
